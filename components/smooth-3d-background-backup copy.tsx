...
function FloatingElement({
  ...
  const [ref, api] = useBox(() => ({
    ...
    linearDamping: 0.4 + (Math.abs(Math.sin(chaosX * 200)) * 0.3), // Varied damping 0.4-0.7
    angularDamping: 0.2 + (Math.abs(Math.cos(chaosY * 200)) * 0.2), // Varied damping 0.2-0.4
  }))

  const meshRef = useRef<THREE.Group>(null!)

  // Apply continuous forces for floating behavior
  useFrame((state) => {
    if (meshRef.current && api) {
      const time = state.clock.elapsedTime + timeOffset

      // Completely desynchronized individual forces
      const uniqueTimeX = time * speed * (0.03 + chaosX * 0.02) + orbitPhase * 3.7
      const uniqueTimeY = time * speed * (0.04 + chaosY * 0.03) + floatPhase * 2.3
      const uniqueTimeZ = time * speed * (0.035 + chaosZ * 0.025) + orbitPhase * 4.1

      const orbitForceX = Math.cos(uniqueTimeX) * (0.08 + Math.abs(chaosX) * 0.04)
      const orbitForceZ = Math.sin(uniqueTimeZ) * (0.08 + Math.abs(chaosZ) * 0.04)
      const floatForceY = Math.sin(uniqueTimeY) * (0.06 + Math.abs(chaosY) * 0.03)

      // Individual random micro-forces to break synchronization
      const microForceX = Math.sin(time * 7.32 + chaosX * 12.7) * 0.02
      const microForceY = Math.cos(time * 5.84 + chaosY * 15.3) * 0.015
      const microForceZ = Math.sin(time * 6.91 + chaosZ * 18.9) * 0.02

      // Get current position for centering force
      const currentPos = ref.current?.position
      if (currentPos) {
        const centeringForceX = -currentPos.x * (0.015 + Math.abs(chaosX) * 0.01)
        const centeringForceY = -currentPos.y * (0.015 + Math.abs(chaosY) * 0.01)
        const centeringForceZ = -currentPos.z * (0.015 + Math.abs(chaosZ) * 0.01)

        api.applyForce([
          orbitForceX + centeringForceX + microForceX,
          floatForceY + centeringForceY + microForceY,
          orbitForceZ + centeringForceZ + microForceZ
        ], [0, 0, 0])
      }

      // Add anti-sticking separation impulses randomly
      if (Math.random() < 0.02) { // 2% chance per frame
        const separationImpulse = 0.1 + (Math.random() * 0.1)
        api.applyImpulse([
        ...
        >
          <Physics
            ...
            defaultContactMaterial={{
              friction: 0.05, // Very low friction to prevent sticking
              restitution: 0.95, // High restitution for bouncy separation
              contactEquationStiffness: 1e6, // Lower stiffness
              contactEquationRelaxation: 4, // Higher relaxation
            }}
            broadphase="Naive"
            allowSleep={false}
            iterations={10} // More solver iterations for stability
          >
            <PhysicsBoundaries />
            <FloatingCodeBlocks />
            <FloatingParticles />
